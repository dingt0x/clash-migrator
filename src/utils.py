...
async def get_url_from_env_list(token :str) -> List[str]:
    url_from_env_list = []
    for i in range(1, 9):
        url_from_env = os.environ.get(f"{token}URL{i}", "").strip()
        if url_from_env:
            url_from_env_list.append(url_from_env)

    return url_from_env_list

async def get_url_and_prefix_list(token: str) -> Tuple[List[str], list[str]]:
    url_list = []
    prefix_list = []
    url_from_env_list=await  get_url_from_env_list(token)
